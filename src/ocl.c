...
cl_mem tex_buffer, cur_buffer, next_buffer, diffCounter;
...
void ocl_init (void)
{
  ...
  diffCounter = clCreateBuffer (context, CL_MEM_READ_WRITE, sizeof(int),
				NULL, NULL);
  if (!diffCounter)
    exit_with_error ("Failed to allocate diffCounter");
